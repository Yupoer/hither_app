-- Fix "future destination cannot be completed" for the first open stop.
--
-- Old rule used max(active_destination.position, any arrival.position) or
-- min(all positions). That blocked:
--   • Day-2 first open stop when Day-1 rows still exist with lower positions
--   • Next stop after complete_gathering_stop cleared active_destination
--   • Local path-plan (no shared session / active destination)
--
-- New rule: mark is allowed unless an earlier same-scope stop is still open
-- for THIS user (not closed, and no personal arrival). First stop always OK.
