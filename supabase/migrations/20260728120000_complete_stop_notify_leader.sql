-- complete_gathering_stop: also notify the acting leader (this-device APNs/FCM).
-- Spec: auto-complete feedback on the leader device; send-push honors
-- target_user_id so the sender is not filtered out of the alert set.
...
begin
  ...
  -- Notify members who have not marked arrived (unchanged).
  for v_member in
    select m.user_id
  ...
  -- This-device: acting leader always gets a targeted push (APNs/FCM) so iOS
  -- auto-complete feedback does not depend on a client-only local banner.
  begin
    perform extensions.notify_push(jsonb_build_object(
      'category', 'journey',
      'group_id', p_group_id,
