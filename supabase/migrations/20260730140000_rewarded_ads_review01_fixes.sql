-- Review-01 fixes for Rewarded Ads Token Store:
-- 1. redeem_store_product accepts stable client request key (idempotent retries)
-- 2. verifying sessions cannot be client-failed (SSV owns terminal state)
-- 3. LA entitlement helpers not executable by authenticated clients

-- ============================================================
-- update_reward_session_status: no verifying → failed from client
-- ============================================================
...
begin
  ...
  -- Once verifying (client reward / SSV pending), only credit RPC or expiry
  -- may terminate. Client must not fail and block a late Google callback.
  if v_row.status = 'verifying' then
    if v_next = 'verifying' then
      return json_build_object(
        'ok', true,
        'already', true,
        'status', 'verifying',
...
-- ============================================================
-- Helpers: revoke client execute (internal SECURITY DEFINER only)
-- ============================================================

revoke all on function public.user_has_live_activity_lifetime(uuid)
  from public, anon, authenticated;
grant execute on function public.user_has_live_activity_lifetime(uuid)
  to service_role;
...
grant execute on function public.effective_live_activity_entitlement(uuid, uuid)
  to service_role;
-- Note: other SECURITY DEFINER RPCs in this schema still call these helpers
-- internally (owner = postgres / same schema); client cannot invoke them.

-- ============================================================
-- redeem_store_product: idempotent client request key
-- ============================================================

-- Drop previous 2-arg signature so PostgREST binds the 3-arg form.
drop function if exists public.redeem_store_product(text, uuid);
...
begin
  ...
  -- Stable external_ref for retries: prefer client key (scoped to user), else random.
  ...
  if v_req_key is not null then
    ...
    if found then
      -- Idempotent replay of a successful debit.
      return json_build_object(
        'ok', true,
        'already', true,
        ...
        'extra_point_credits', case
          when p_group_id is not null then (
            select balance from public.trip_extra_point_credits where group_id = p_group_id
          )
          else null
        end,
...
exception
  when unique_violation then
    -- Concurrent retry of same client key — return prior ledger row if present.
    if v_req_key is not null then
      select * into v_prior
      from public.token_ledger
