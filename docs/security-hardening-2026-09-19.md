...
The reviewed security migration is now applied in Hither production and recorded in migration history. `sync-app-store-subscription` is deployed there as `ACTIVE` with `verify_jwt=true`; a no-auth HTTP smoke check returned `401`, and the production ACL query confirmed client premium-field writes and unauthorized membership writes are denied. The pre-existing remote migration history drift still blocks a normal `db push` dry-run, so unrelated pending migrations were not applied or repaired. No credential rotation, Git history rewriting, native build, or store submission was performed.
