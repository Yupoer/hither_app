...
**Blocked by:** 08 — 收斂文件、設定、support-site 與所有無主 WIP。

**Status:** ready-for-agent

- [ ] 執行各切片 focused Jest、完整 Jest、TypeScript typecheck、diff check 與相鄰 regression suites，保存命令、版本、結果與失敗輸出。
- [ ] 執行 Deno tests、pgTAP／Supabase runtime tests；無 runtime 或 credentials 時標成 Unverified／Blocked，不以 source-string contract 取代。
...
- [ ] Lint baseline 與本次新增問題分開；任何與本次變更相關的 Failed 都有處置或阻止 merge。
- [ ] Gate matrix 分開列 native build、server deploy、migration、OTA、TestFlight 與 App Store submit，不把其中一項成功推論成其他項成功。
- [ ] WIP manifest coverage 為完整，所有項目均有最終 disposition，才可讓 Ticket 10 開始。
