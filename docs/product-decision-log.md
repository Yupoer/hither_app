...
### 2026-08-22 設定 sheet、訂閱全螢幕、商店重構、路線順序、原生 Toggle、頭像（已確認外觀，尚未實作）

下一包 GitHub parent 必須覆寫 #220／#225／#229 的**對外行為**。模擬與逐條決策：`hither/.lavish/hither-target-ux/MEMORY.md`。

- **開關與分頁（覆寫 #225；已確認：只換開關、改測試）**：
  - 四分頁 = **iOS 原生 Segmented Control**（不是 Switch，不是自繪 `SheetPaneTabs`）。對齊 Image #1；自繪與原生衝突時原生勝出。舊測試必須改寫，不得再要求 `SheetPaneTabs`。
  - 開／關列：**只換右邊那顆**為 Settings.app 系統 Toggle（`@expo/ui` SwiftUI Toggle）。列維持現有玻璃列，本包不做系統 List。不是 RN `NativeSwitch`。系統綠 `#30D158`，不塗品牌色。舊 `NativeSwitch` 字串測試必須改寫。
- **頭像（已確認：群組頭像）**：
  - 成員頭像 = 你這個人（`profiles`），列表與進團同一顆。已知 userId 禁止字母／數字 initials。
  - 群組頭像 = 這個團的大頭貼（`groups.avatar`／`avatar_color`），每團獨立。Leader 設定。禁止拿 Leader 個人頭像冒充。列表主圖與進團團名旁都顯示群組頭像。
- **商店重構**：商店最上方展示與設定同款的「大看板橫幅按鈕」（未訂閱者專屬，直通全螢幕訂閱）；商品重構為精緻雙欄網格（圖示、品名、簡述、點數兌換按鈕），廢除臃腫單一大格。
- **路線編輯順序**：
  - **左上角 3 態切換**：保留 `≡ 拖曳模式` → `☑ 多選模式` → `🔒 鎖定模式` 循環切換契約。
  - **頂部工具列（無 Emoji）**：「天數與日期 / 常用地點 / 匯入」採用俐落向量圖示，收整為**單行工具列**（徹底解決 Image #4 換行破版）。
  - **天數列與住宿按鈕**：天數標籤右側緊鄰**靠左對齊**的「設定住宿」向量膠囊按鈕。
  - **停靠點格數與比例**：對齊 Image #3（趨勢設定）標準高度（52~54px）與 15px 字體比例，**格內僅顯示停靠點名稱**，去除過量 Emoji 與多餘雜訊；右側保留 44px 寬裕熱區的拖曳把手與多選勾選鈕。
- **設定 Sheet 手勢與動作分流**：
  - 入口為 bottom sheet（預設半開，左右留 10px）。半開時往下捲／往上拖一定先全開（貼齊螢幕邊緣）。
  - **子頁面**：點擊設定項目彈出**預設半開的子 bottom sheet**，用戶可手動拖曳／滾動將其展開為全開，或按右上角叉叉 `✕` 關閉。禁止 settings 內 stack push。
  - **動作類**：`回到主畫面`（關閉設定並直接跳轉）、`離開群組` 與 `登出`（直接觸發確認對話框 Alert，不彈任何 Sheet）。
- **訂閱**：從下蓋滿的**全螢幕**（無 grabber、不是 sheet）。修正文字斷行，結構與 FocusFlight 一致，文案用 Hither。
## 尚未定案
